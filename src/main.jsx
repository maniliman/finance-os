          ...
          <div className="fixed inset-0 z-[110] bg-[#05070a]/95 backdrop-blur-xl flex flex-col animate-in fade-in">
            <div className="px-6 pt-12 pb-4 border-b border-[#1e2532] bg-[#11151f] space-y-5">
               ...
               <div className="flex gap-2 overflow-x-auto pb-2">
                 <button onClick={() => toggleLedger('all')} className={`rounded-xl uppercase tracking-widest px-4 py-2 text-[10px] font-black ${activeLedgers.has('all') ? 'bg-mint text-black' : 'bg-[#05070a] border border-[#1e2532] text-slate-500'}`}>All</button>
                 <button onClick={() => toggleLedger('fiduciary')} className={`rounded-xl uppercase tracking-widest px-4 py-2 text-[10px] font-black ${activeLedgers.has('fiduciary') ? 'bg-fiduciary text-white' : 'bg-[#05070a] border border-[#1e2532] text-slate-500'}`}>Fiduciary</button>
               </div>
            </div>
            <div className="flex-1 overflow-y-auto p-6"><button onClick={() => setIsSearching(false)} className="w-full py-4 rounded-2xl border border-[#1e2532] bg-[#1a202c] text-[11px] font-black uppercase text-slate-300 tracking-[0.2em] flex items-center justify-center gap-2">Close Search</button></div>
          </div>
        )}

        <nav className="fixed bottom-0 left-0 right-0 max-md mx-auto px-12 py-6 flex items-center justify-between bg-[#05070a]/90 backdrop-blur-2xl border-t border-[#1e2532] z-40">
